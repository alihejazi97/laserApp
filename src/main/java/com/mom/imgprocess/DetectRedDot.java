...
//        Imgproc.circle(matShow,);
    ...
    void drawCircles(Mat mat, Size original) {
        for (int i = shotPoint.size() - 1; i >= 0; i--) {
        ...
        if (!shotPoint.isEmpty()) {
            ...
            Imgproc.circle(mat, shotPoint.get(shotPoint.size() - 1), (int) (showCircleR * mat.size().width), new Scalar(0, 0, 255), -1);
        }
    }
